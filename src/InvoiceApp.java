 ...
 * Jayden Lueders
 * version 2.0 update this to version 2
 * link https://github.com/jdl699/Invoice_App/tree/master
 */
public class InvoiceApp {
    ...
    /**
     * work out the disocunt that needs to be applied
     * this returns the discount percent and will be called by the print receipt class later
     */

    /**
     * Method for calculating the discount percent of the subtotal.
     *
     * @param subTotal Line item subtotal.
     * @return Discount percent for the line item based on the subtotal.
     */
    ...
    private static void printReceipt(List<String> descriptionList, List<Integer> qtyList, List<Double> priceList) {
        ...
        for (int i = 0; i < descriptionList.size(); i++) {
            ...
            System.out.println();
            System.out.printf("%-20s  %3d  @  $%,8.2f = $%,8.2f\n", description, qty, price, subTotal);

            if (discountPercent > 0) {
                System.out.printf("%20s  %,2.0f%%   -$%,8.2f = $%,8.2f\n", "Discount", discountPercent * 100, discountAmount, subTotal - discountAmount);
            }


            savingsTotal += discountAmount;
            receiptTotal += subTotal - discountAmount;
